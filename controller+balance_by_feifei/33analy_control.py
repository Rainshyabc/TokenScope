...
with open(indexresult_path+'eventmap', 'r') as f02:
    for line02 in f02:
        a02 = line02.strip('\n').split('#')
        eventsc = a02[0]
        # mapdic = eval(a02[1])
        # EventMap[eventsc] = mapdic
        mapdic = a02[1]
        Controller[eventsc] = (mapdic, eventsc)
        Controller[mapdic] = (mapdic, eventsc)
...
def analy_trace(filename):
    ...
    with open(datalog_path+filename, 'r') as f2:
        for line2 in f2:
            ...
            # #test
            # if txhash != 'a2e40ea5121329fdf3f501411b605925aa1f9fa51b64ac4ee6563788e02a0699':
            #     continue
            ...
            for ly in LayerSC:
                ...
                if now_token in Controller:
                    control_tuple = Controller[now_token]
                    maptoken = control_tuple[0]
                    eventtoken = control_tuple[1]
                    keytype = 'none'
                    if now_token == maptoken:
                        keytype = 'map'
                    ...
                    if control_tuple not in Analy[trace_count]['controller']:
                        Analy[trace_count]['controller'][control_tuple] = {}
                    if keytype not in Analy[trace_count]['controller'][control_tuple]:
                        Analy[trace_count]['controller'][control_tuple][keytype] = {}
                    if ly not in Analy[trace_count]['controller'][control_tuple][keytype]:
                         Analy[trace_count]['controller'][control_tuple][keytype][ly] = []
                    for addr in addrlist:
                        value = addrlist[addr]
                        Analy[trace_count]['controller'][control_tuple][keytype][ly].append((addr, value))
